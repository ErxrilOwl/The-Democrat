	...
	<body>
		...
		<div class="main-news">
			<div class="main-news-header">
				<h2 id="hot-news">UNC HOT SEAT</h2>
			</div>
				...
				<div class="news-col news-col-1">
					<section class="news-latest">
						<?php 
							...
							$query = "SELECT * FROM tblarticle WHERE type = 'hot-seat' ORDER BY date_submitted DESC LIMIT $start, $per_page";
					...
					<?php 
						$query = "SELECT * FROM tblarticle WHERE type = 'hot-seat'";
							$result = mysqli_query($conn, $query);
							$total_records = mysqli_num_rows($result);
							$total_pages = ceil($total_records/$per_page);

						echo "<div class='row row-page row-btn-profile'>".
								"<a href='unc-hot-seat.php?page=1'><button class='special'>First Page</button></a>";
						...
						if($total_pages > 5){
							if($page > 2){
								...
								for($i = $lower; $i <= $upper; $i++){
									if($i == $page){
										echo "<a href='unc-hot-seat.php?page=".$i."'><button class='current'>".$i."</button></a>";	
									}
									else{
										echo "<a href='unc-hot-seat.php?page=".$i."'><button>".$i."</button></a>";	
									}
								}			
							}
							else{
						...
						echo "<a href='unc-hot-seat.php?page=".$total_pages."'><button class='special'>Last Page</button></a></div>";
