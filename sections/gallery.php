			...
			<header class="title-bar">
				...
				<div class="row nav-menu title-col-2">
					...
					<ul class="menu">
						...
						<span class="asterisk">*</span>
						<li class="menu-btn"><a class="nav-link-effect nav-before nav-after" href="../sections/view-gallery.php">GALLERY</a></li>
						<span class="asterisk">*</span>
				...
				<div class="row row-main">
					...
					<?php 
						...
						if(mysqli_num_rows($result) > 0){
							while($row = mysqli_fetch_assoc($result)){
								$varCName = $row['collection_name'];
								$varDate = $row['date_created'];

								$imgQuery ="SELECT * FROM tblcollectionimage WHERE collection_id ='".$row["id"]."'";
								$imgResult =  mysqli_query($conn, $imgQuery);

								echo "<a href='view-gallery.php?page=1&collectionId=".$row["id"]."'><div class='image-block'><div class='image-box'><div class='image'>";

								if(mysqli_num_rows($imgResult)>0){
										while($rowImg = mysqli_fetch_assoc($imgResult)){
										$varImg = "<img height='300' width='300' src='data:image;base64,".$rowImg['image']."'>";
										}
										echo $varImg;
								}
								else{
								...
								echo "<p>".$varCName."</p></div></div></div></a>";
							}
						}
					?>
					</div>
					<?php 
						...
						echo "<div class='row row-page row-btn-profile'>".
								"<a href='gallery.php?page=1'><button class='special'>First Page</button></a>";
						...
						if($total_pages > 5){
							if($page > 2){
								...
								for($i = $lower; $i <= $upper; $i++){
									if($i == $page){
										echo "<a href='gallery.php?page=".$i."'><button class='current'>".$i."</button></a>";	
									}
									else{
						...
						echo "<a href='gallery.php?page=".$total_pages."'><button class='special'>Last Page</button></a></div>";
