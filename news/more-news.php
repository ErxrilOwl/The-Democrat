				...
				<div class="news-col news-col-1">
					<section class="news-latest">
						<?php 
							...
							$query = "SELECT * FROM tblarticle WHERE type = 'news' ORDER BY date_submitted DESC LIMIT $start, $per_page";
							...
							if(mysqli_num_rows($result) > 0){
								while($row = mysqli_fetch_assoc($result)){
									...
									echo  "<a href='../sections/view.php?articleId=".$row['id']."'><button class='button'><span>Read More</span></button></a></div></article>";
					...
					<?php 
						$query = "SELECT * FROM tblarticle WHERE type = 'news'";
							$result = mysqli_query($conn, $query);
							$total_records = mysqli_num_rows($result);
							$total_pages = ceil($total_records/$per_page);
							
						echo "<div class='row row-page row-btn-profile'>".
								"<a href='more-news.php?page=1'><button class='special'>First Page</button></a>";
						...
						if($total_pages > 5){
							if($page > 2){
								...
								for($i = $lower; $i <= $upper; $i++){
									if($i == $page){
										echo "<a href='more-news.php?page=".$i."'><button class='current'>".$i."</button></a>";	
									}
									else{
										echo "<a href='more-news.php?page=".$i."'><button>".$i."</button></a>";	
									}
								}			
							}
							else{
						...
						echo "<a href='more-news.php?page=".$total_pages."'><button class='special'>Last Page</button></a></div>";
